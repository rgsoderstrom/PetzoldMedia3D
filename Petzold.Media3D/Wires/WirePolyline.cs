...
namespace Petzold.Media3D
{
    ...
    public class WirePolyline : WireBase
    {
        ...
        {
            get { return (int)GetValue (DecimationProperty); }
            set { SetValue (DecimationProperty, value); }
        }

        // Using a DependencyProperty as the backing store for Decimation.  This enables animation, styling, binding, etc...
        public static readonly DependencyProperty DecimationProperty =
            DependencyProperty.Register ("Decimation", typeof (int), typeof (WirePolyline), new PropertyMetadata (1, PropertyChanged), ValidateDecimation);

        static bool ValidateDecimation (object obj)
        {
            int d = (int) obj;
            return d > 0;
        }
                                         ...
                                         Point3DCollection lines)
        {
            ...
            for (int i = 0; i < points.Count - 1; i+=Decimation)// i+=2) // i++)
            {
                lines.Add(points[i]);
